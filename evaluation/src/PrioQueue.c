...
void printPrioQueue(PrioQueue *queue)
{
    ...
    int pos = 0;
    printf("|pos|node|prio|status|:\n");
    while(pos <= queue->last_elem_pos){
        int node_index = queue->node[pos];
        printf("|%d|%d|%f|%d| \t", pos, node_index, queue->prio[node_index], queue->state[node_index]);
        pos++;
    }
    printf("\n\n");
}
