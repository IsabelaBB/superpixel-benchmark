...
def test(args, model, img_paths, save_path, spixeIds, idx,scale):
    ...
    print(H_, W_, scale, args.input_img_height, args.input_img_width, img1.shape)
    ...
    #spixel_viz = np.array((spixel_viz - np.min(spixel_viz)) / (np.max(spixel_viz) - np.min(spixel_viz)))
    ...
    tmp = (spixel_label_map + 1).astype(np.uint16)
...
def main():
    ...
    # 0.2, 0.3, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4
    for scale in [0.2, 0.3, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]:
        assert (input_img_height * scale % 16 == 0 and input_img_width * scale % 16 == 0)
        save_path = args.output + '/{0}'.format(int(input_img_height / 16 * scale * input_img_width / 16 * scale))
        ...
        with open(args.test_list, 'r') as tf:
            ...
            for id in img_path:
                img_path = os.path.join(data_dir,'img/%.8d.jpg'%int(id))
