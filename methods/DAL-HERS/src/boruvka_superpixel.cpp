...
BoruvkaSuperpixel::build_2d(int shape_x, int shape_y,
        int n_feature_channels, T const *feature, T const *affinities, T const *border_strength, int8_t const *edgeless, int32_t const *seed)
{
...
#define INSTANTIATE(T)                                          \
    template void BoruvkaSuperpixel::build_2d<T>                \
        (int, int, int, T const *, T const *, T const *,        \
...
template <typename T> void
BoruvkaSuperpixel::build_common(int n_edge_store, T const *feature, T const *affinities,
