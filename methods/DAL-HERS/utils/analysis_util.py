...
def ProduceAffMap(img_file, model):
    ...
    #img_ = cv2.imread(img_file)[:, :, ::-1]
    #img_ = transforms.CenterCrop(size=200)(Image.fromarray(img_))
    #img_ = np.asarray(img_)
    #img_ = img_.astype(np.float32) # may get 4 channel (alpha channel) for some format
